...
namespace Basf.Repository
{
    public class RepositoryHelper
    {
        ...
        internal static string GetPagingCache(int hashKey, string connString, string sql, int pageIndex, int? pageSize, string orderBy, IOrmProvider provider)
        {
            ...
            {
                var pagedSql = provider.GetPagingExpression(pageIndex, pageSize, orderBy);
                result = (HasUnionRegex.IsMatch(sql) ? "SELECT * FROM (" + sql + ") PageList " : sql) + pagedSql;
        ...
        internal static Action<IDbCommand, TEntity> CreateUpdateParametersHandler<TEntity>(string paramPrefix, EntityMapper mapper, List<string> updateProperties)
        {
            var dm = new DynamicMethod("CreateUpdateParameter_" + Guid.NewGuid().ToString(), null, new[] { typeof(IDbCommand), mapper.EntityType }, mapper.EntityType, true);
            ILGenerator il = dm.GetILGenerator();
            ...
            {
                CreateParamter(il, paramPrefix, mapper.MemberMappers[propName], true);
            }
