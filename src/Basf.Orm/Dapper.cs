...
namespace Basf.Orm
{
    ...
    public struct CommandDefinition
    {
        ...
        {
            if (parameters is Dapper.IParameterCallbacks)
            {
                ((Dapper.IParameterCallbacks)parameters).OnCompleted();
            }
        }
        /// <summary>
        ...
        public CommandDefinition(string commandText, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null,
            CommandType? commandType = null, CommandFlags flags = CommandFlags.Buffered, CancellationToken cancellationToken = default(CancellationToken))
        ...
        static Dapper.Link<Type, Action<IDbCommand>> commandInitCache;
        static Action<IDbCommand> GetInit(Type commandType)
        {
            if (commandType == null) return null; // GIGO
            Action<IDbCommand> action;
            if (Dapper.Link<Type, Action<IDbCommand>>.TryGet(commandInitCache, commandType, out action))
            {
                return action;
            }
            ...
            // cache it            
            Dapper.Link<Type, Action<IDbCommand>>.TryAdd(ref commandInitCache, commandType, ref action);
            return action;
        }
...
#pragma warning restore 618
        ...
        public partial class Identity : IEquatable<Identity>
        {
            ...
            private Identity(string sql, CommandType? commandType, string connectionString, Type type, Type parametersType, Type[] otherTypes, int gridIndex)
            {
                ...
                unchecked
                {
                    ...
                    hashCode = hashCode * 23 + (connectionString == null ? 0 : Dapper.connectionStringComparer.GetHashCode(connectionString));
            ...
            public bool Equals(Identity other)
            {
                return
                    ...
                    Dapper.connectionStringComparer.Equals(connectionString, other.connectionString) &&
        ...
        internal static readonly MethodInfo format = typeof(Dapper).GetMethod("Format", BindingFlags.Public | BindingFlags.Static);
        /// <summary>
        ...
        internal static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
        {
            ...
            foreach (var prop in props)
            {
                ...
                if (dbType == DynamicParameters.EnumerableMultiParameter)
                {
                    ...
                    il.EmitCall(OpCodes.Call, typeof(Dapper).GetMethod("PackListParameters"), null); // stack is [parameters]
                    continue;
                }
                ...
                if (checkForDuplicates)
                {
                    ...
                    il.EmitCall(OpCodes.Call, typeof(Dapper).GetMethod("FindOrAddParameter"), null); // stack is [parameters] [parameter]
                }
                else
                {
                ...
                if (dbType != DbType.Time && handler == null) // https://connect.microsoft.com/VisualStudio/feedback/details/381934/sqlparameter-dbtype-dbtype-time-sets-the-parameter-to-sqldbtype-datetime-instead-of-sqldbtype-time
                {
                    ...
                    if (dbType == DbType.Object && prop.PropertyType == typeof(object)) // includes dynamic
                    {
                        ...
                        il.Emit(OpCodes.Call, typeof(Dapper).GetMethod("GetDbType", BindingFlags.Static | BindingFlags.Public)); // stack is now [parameters] [[parameters]] [parameter] [parameter] [db-type]
                    }
                    else
                    {
...
#pragma warning disable 618
            ...
            { // this *does* need special handling, though
                return r => Dapper.ReadChar(r.GetValue(index));
            }
            if (type == typeof(char?))
            {
                return r => Dapper.ReadNullableChar(r.GetValue(index));
            }
            if (type.FullName == LinqBinary)
            {
        ...
        public static Func<IDataReader, object> GetTypeDeserializer(Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false)
        {
            ...
            foreach (var item in members)
            {
                if (item != null)
                {
                    ...
                    {
                        il.EmitCall(OpCodes.Call, typeof(Dapper).GetMethod(
...
#pragma warning restore 618
            ...
            il.EmitCall(OpCodes.Call, typeof(Dapper).GetMethod("ThrowDataException"), null);
        ...
        public partial class GridReader : IDisposable
        {
            ...
            internal GridReader(IDbCommand command, IDataReader reader, Identity identity, Dapper.IParameterCallbacks callbacks, bool addToCache)
            {
            ...
            private IEnumerable<TReturn> MultiReadInternal<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(Delegate func, string splitOn)
            {
                ...
                try
                {
                    foreach (var r in Dapper.MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(null, default(CommandDefinition), func, splitOn, reader, identity, false))
                    {
                        yield return r;
                    }
                }
            ...
            private Dapper.IParameterCallbacks callbacks;
    ...
    /// </summary>
    partial class DynamicParameters : Dapper.IDynamicParameters, Dapper.IParameterLookup, Dapper.IParameterCallbacks
    {
        internal const DbType EnumerableMultiParameter = (DbType)(-1);
        static Dictionary<Dapper.Identity, Action<IDbCommand, object>> paramReaderCache = new Dictionary<Dapper.Identity, Action<IDbCommand, object>>();

        Dictionary<string, ParamInfo> parameters = new Dictionary<string, ParamInfo>();
        List<object> templates;

        object Dapper.IParameterLookup.this[string member]
        {
            get
            {
                ParamInfo param;
        ...
        void Dapper.IDynamicParameters.AddParameters(IDbCommand command, Dapper.Identity identity)
        {
        ...
        /// <param name="command">The raw command prior to execution</param>
        /// <param name="identity">Information about the query</param>
        protected void AddParameters(IDbCommand command, Dapper.Identity identity)
        {
            var literals = Dapper.GetLiteralTokens(identity.sql);
            ...
            if (templates != null)
            {
                foreach (var template in templates)
                {
                    ...
                    lock (paramReaderCache)
                    {
                        ...
                        {
                            appender = Dapper.CreateParamInfoGenerator(newIdent, true, RemoveUnused, literals);
            ...
            foreach (var param in parameters.Values)
            {
                ...
                var isCustomQueryParameter = val is Dapper.ICustomQueryParameter;

                Dapper.ITypeHandler handler = null;
                if (dbType == null && val != null && !isCustomQueryParameter) dbType = Dapper.LookupDbType(val.GetType(), name, true, out handler);
                if (dbType == DynamicParameters.EnumerableMultiParameter)
                {
#pragma warning disable 612, 618
                    Dapper.PackListParameters(command, name, val);
#pragma warning restore 612, 618
                }
                else if (isCustomQueryParameter)
                {
                    ((Dapper.ICustomQueryParameter)val).AddParameter(command, name);
                }
                else
                {
                    ...
                    if (handler == null)
                    {
                        p.Value = Dapper.SanitizeParameterValue(val);
            ...
            if (literals.Count != 0) Dapper.ReplaceLiterals(this, command, literals);
        }
        ...
        MAKECALLBACK:
            (outputCallbacks ?? (outputCallbacks = new List<Action>())).Add(() =>
            {
                ...
                {
                    Dapper.ITypeHandler handler;
                    dbType = (!dbType.HasValue) ? Dapper.LookupDbType(targetMemberType, targetMemberType.Name, true, out handler) : dbType;
        ...
        void Dapper.IParameterCallbacks.OnCompleted()
        {
...
#if !DNXCORE50
    sealed class DataTableHandler : Dapper.ITypeHandler
    {
    ...
    /// Used to pass a DataTable as a TableValuedParameter
    /// </summary>
    sealed partial class TableValuedParameter : Dapper.ICustomQueryParameter
    {
        ...
        void Dapper.ICustomQueryParameter.AddParameter(IDbCommand command, string name)
        {
        ...
        {
            parameter.Value = Dapper.SanitizeParameterValue(table);
            if (string.IsNullOrEmpty(typeName) && table != null)
            {
                typeName = Dapper.GetTypeName(table);
            }
            if (!string.IsNullOrEmpty(typeName))
            {
    ...
    /// </summary>
    sealed partial class DbString : Dapper.ICustomQueryParameter
    {
        /// <summary>
        ...
        public void AddParameter(IDbCommand command, string name)
        {
            ...
            var param = command.CreateParameter();
            param.ParameterName = name;
            param.Value = Dapper.SanitizeParameterValue(Value);
    ...
    /// </summary>
    sealed partial class SimpleMemberMap : Dapper.IMemberMap
    {
    ...
    /// </summary>
    sealed partial class DefaultTypeMap : Dapper.ITypeMap
    {
            ...
            foreach (ConstructorInfo ctor in constructors.OrderBy(c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1)).ThenBy(c => c.GetParameters().Length))
            {
                ...
                for (; i < ctorParameters.Length; i++)
                {
                    if (!String.Equals(ctorParameters[i].Name, names[i], StringComparison.OrdinalIgnoreCase))
                        break;
                    if (types[i] == typeof(byte[]) && ctorParameters[i].ParameterType.FullName == Dapper.LinqBinary)
                        continue;
    ...
    /// </summary>
    sealed partial class CustomPropertyTypeMap : Dapper.ITypeMap
    {
        private readonly Type _type;
